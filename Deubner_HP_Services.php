...
define( 'DEUBNER_HP_SERVICES_VERSION', '0.15.1' );
...
/** @var string Alpine.js-Vendor-Version (lokal gebundled, seit 0.15.1). */
...
/*
|--------------------------------------------------------------------------
| Component-System-Helpers (v0.15.1)
|--------------------------------------------------------------------------
|
...
function dhps_init() {
    ...
    // 3b. Component-Registry: UI-Bausteine registrieren (v0.15.1).
    ...
    // 4b. MMB-Lazy-Akkordeon AJAX-Endpoint (v0.15.1).
    $mmb_ajax = new DHPS_MMB_AJAX_Handler( $client, $cache );
    $mmb_ajax->register();

    // 4c. Admin-Dashboard REST-API (v0.15.1): Service-Health, API-Test, Cache-Stats.
    ...
    $admin_rest       = new DHPS_Admin_REST( $client, $cache, $health_collector, $cache_stats );
    $admin_rest->register();
    ...
    // 7b. TPT-Modules-Layer (v0.15.1): reichert $data um Admin-Texte an,
...
function dhps_enqueue_frontend_styles() {
    ...
    // Seit 0.15.1. Wird sofort geladen, weil die Components ueberall potentiell verwendet werden.
    wp_register_style(
        'dhps-components-css',
    ...
    // Elementor-Atomic-Token-Bridge (seit 0.15.1, optional, Default aus).
    ...
    // MAES-Aktuelles JS-Block in v0.13.1 -> v0.15.1 ausgelagert,
    // in v0.15.1 obsolet: ContentCard's Alpine-Toggle uebernimmt die
    // Akkordeon-Funktion. File geloescht, Enqueue entfernt.

    // Alpine.js v3.x Vendor (lokal gebundled, defer, conditional enqueue, seit 0.15.1).
    wp_register_script(
...
/*
|--------------------------------------------------------------------------
| Component-Registry-Bootstrap (v0.15.1)
|--------------------------------------------------------------------------
|
...
function dhps_register_components() {
    ...
    DHPS_Component_Registry::register( 'content-card', array(
        'default_props'   => array(
            ...
            'class'       => '',
            'service'     => '',
            'data_attrs'  => array(),  // seit 0.15.1 - data-* fuer Player-Selectoren etc.
        ),
        'stateful'        => true,
...
/*
|--------------------------------------------------------------------------
| Alpine.js Conditional Loading (v0.15.1)
|--------------------------------------------------------------------------
|
 ...
 *
 * Seit 0.15.1. Aktuell nur Bridge-Toggle, weitere Optionen wandern bei
 * Gelegenheit (z.B. v0.15.1 Admin-Dashboard) hierher.
 *
 * @since 0.15.1
 *
 * @return void
...
/*
|--------------------------------------------------------------------------
| Admin-Dashboard React-Bundle (v0.15.1)
|--------------------------------------------------------------------------
|
...
function dhps_enqueue_admin_dashboard( $hook_suffix ) {
    ...
    // QA-Fix v0.15.1 Critical-1: vorheriger Hard-Code 'toplevel_page_dhps_dashboard'
